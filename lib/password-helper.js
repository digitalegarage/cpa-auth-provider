...
registerQuality(
    'no',
    {
        ...
        getWeaknessesErrors: (mail, password, req) => {
            return [];
        }
    }
);
...
registerQuality(
    'owasp',
    {
        ...
        getWeaknessesErrors: (mail, password, req) => {
            ...
            let toReturn = [];
            for (let i = 0; i < owaspResult.failedTests.length; i++) {
                toReturn.push({code: "OWASP_" + owaspResult.failedTests[i], message: req.__('BACK_SIGNUP_PASS_STRENGTH_' + owaspResult.failedTests[i])});
            }
            if (mail && mail.toUpperCase() === password.toUpperCase()) {
                toReturn.push({code: "CANNOT_USE_EMAIL_AS_PASSWORD", message: req.__('BACK_SIGNUP_PASS_STRENGTH_EMAIL_EQUALS_PASSWORD')});
            }
            return toReturn;
        },
    }
...
registerQuality(
    ...
    {
        isStrong: (mail, password) => {
            let minimalExpectedScore = 0;
            if (config.password.minimalExpectedScore){
                minimalExpectedScore = config.password.minimalExpectedScore;
            }
            let result = simplePassword.getQuality(password).value > minimalExpectedScore;
        ...
        getWeaknessesErrors: (mail, password, req) => {
            ...
            if (mail && mail.toUpperCase() === password.toUpperCase()) {
                message.push({code: "CANNOT_USE_EMAIL_AS_PASSWORD", message: req.__('BACK_SIGNUP_PASS_STRENGTH_EMAIL_EQUALS_PASSWORD')});
            }
            ...
            if (result.value > 0) {
                ...
                message.push({code: "PASSWORD_SCORE_TOO_SMALL", message: req.__('BACK_SIGNUP_PASS_SIMPLE_TOO_SIMPLE')});
                return message;
            }
        }
    }
...
function getWeaknessesErrors(mail, password, req) {
    return qualities[PASSWORD_QUALITY_CHECK || DEFAULT_QUALITY].getWeaknessesErrors(mail, password, req);
}
...
module.exports = {
    ...
    getWeaknessesErrors: getWeaknessesErrors
};
