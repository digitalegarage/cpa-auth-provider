...
function getFacebookStrategy(callBackUrl) {
    ...
    logger.debug('reauthenticate with config', config);
    if (config.identity_providers && config.identity_providers.facebook && config.identity_providers.facebook.reauthenticate) {
        fbConfig.authType = "reauthenticate";
        logger.debug('Will reauthenticate since value is', config.identity_providers.facebook.reauthenticate);
