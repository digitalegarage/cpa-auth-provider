...
authHelper.ensureCpaAuthenticated = function (req, res, next) {
    if (req.user) {
        return next();
    }
    res.sendStatus(401);
...
authHelper.authenticateFirst = function (req, res, next) {
    ...
    if (req.query.redirect) {
        query = '?redirect' + encodeURIComponent(req.query.redirect);
    } else {
        query = "?redirect=" + encodeURIComponent(req.originalUrl);
    }
    requestHelper.redirect(res, '/responsive/login' + query);
};
