...
var options = {
    ...
    apis: ['./routes/api/v2/auth/login-resource.js', './routes/api/v2/user/user-resource.js', './routes/api/v2/user/profile-resource.js'], // Path to the API docs
};
...
// DEBUG
app.get('/session/deleteAll', function(req,res,next) {
    appHelper.destroySessionsByUserId(req.user.dataValues.id)
    .then(function() {
        res.sendStatus(200);
    })
    .catch(function() {
...
app.get('/deleteAllButCurrentSession', function(req,res,next) {
...
app.all('*', function (req, res, next) {
    ...
    if (req.query.redirect) {
        if (req.url.indexOf('/api/v2/auth/google') == 0 || req.url.indexOf('/api/v2/auth/facebook') == 0) {
            // remove possible remaining value from aborted request (also secure about fishing)
