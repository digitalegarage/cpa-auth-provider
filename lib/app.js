...
var options = {
    ...
    apis: ['./routes/api/v2/user/profile-resource.js', './routes/api/v2/user/user-resource.js', './routes/api/v2/login/login-resource.js'], // Path to the API docs
};
...
app.use(function (req, res, next) {
    ...
    if (config.session_authorization_header_qualifier
        ...
        if (!cookies) {
            req.headers.cookie = config.auth_session_cookie.name + '=' + req.headers[config.session_authorization_header_qualifier];
        } else {
...
require('../routes/api/v2/login/login-resource')(router);
