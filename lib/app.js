    ...
    switch (config.iframes.option.toLowerCase()) {
      ...
      case 'allow-from':
        if (config.iframes.allow_from_domain) {
          ...
          logger.error("IFRAME OPTIONS: ALLOW-FROM is set, but no allow_from_domain was given!");
          option_string = 'DENY'; // safe fallback?
        }
        break;
      default:
        logger.warn("Cannot set your option '" + config.iframes.option + "', will deny all.");
        option_string = 'DENY';
    }
