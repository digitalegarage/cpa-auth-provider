...
//TODO what do we do with Sentry ?
// // Error handling
...
app.use(function(err, req, res, next) {
    if (err.applicationError) {
        ...
        let error = apiErrorHelper.buildError(err.status || 500, 'UNEXPECTED_ERROR', err.message, 'Unexpected error').applicationError;
        if (req.app.get('env') === 'development') {
...
var options = {
    ...
    apis: ['./lib/api-error-helper.js', './routes/api/v2/auth/login-resource.js', './routes/api/v2/user/user-resource.js', './routes/api/v2/user/profile-resource.js', './routes/api/v2/auth/facebook-resource.js', './routes/api/v2/auth/google-resource.js'], // Path to the API docs
};
...
app.use(function (req, res, next) {
    ...
    if (config.session_authorization_header_qualifier &&
        ...
        logger.debug("req.headers.cookie:", req.headers.cookie);
    }
    next();
});
...
app.use(function(req,res,next) {
    ...
    return parsedUrl.pathname.startsWith('/api/v2/all/nameByUid') ? next() : normalSession(req,res,next);
});
...
require('../routes/email/change-email-resource')(router);
