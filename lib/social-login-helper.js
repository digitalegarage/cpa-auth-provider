    ...
    db.SocialLogin.findOne({
        ...
        if (!socialLogin) {
            ...
            db.LocalLogin.findOne({
                where: db.sequelize.where(db.sequelize.fn('lower', db.sequelize.col('login')), {[Op.like]: profile.email}),
                include: [db.User]
                ...
                if (localLogin) {
                    ...
                    return db.SocialLogin.findOne({
                        where: {[Op.and]: [{email: profile.email}, {email: {[Op.ne]: null}}, {email: {[Op.ne]: ''}}]},
                        include: [db.User]
                    }).then(function (socialLogin) {
                        if (socialLogin) {
                ...
                if (localLogin) {
                    ...
                    db.SocialLogin.findOne({
                        ...
                        if (existingSocialLogin) {
                            ...
                            // Does the user have another social login?
                            return db.SocialLogin.findOne({
                                where: {[Op.and]: [{email: email}, {email: {[Op.ne]: null}}, {email: {[Op.ne]: ''}}]},
                                include: [db.User]
                            }).then(function (socialLogin) {
                                if (socialLogin) {
                                    user = socialLogin.User;
