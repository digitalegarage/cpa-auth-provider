...
module.exports = {
    ...
    afterSocialLoginSucceed: afterSocialLoginSucceed,
    FB: FB,
    GOOGLE: GOOGLE,
    APPLE: APPLE
};
...
function performLogin(profile, socialNetworkName, clientId, done) {
    db.SocialLogin.findOne({
        ...
        var user;
        if (!socialLogin && profile.email) {
            // First we try to find any other login the user might have:
            // Does the user have a local login?
            finder.findUserByLocalAccountEmail(profile.email).then(function (localLogin) {
                if (localLogin) {
                    user = localLogin.User;
                } else {
                    if (profile.email) {
                        // Does the user have another social login?
                        return finder.findUserBySocialAccountEmail(profile.email).then(function (socialLogin) {
                            if (socialLogin) {
                ...
                return db.sequelize.transaction(function (transaction) {
                    ...
                    var date_of_birth_ymd;

                    if (profile.birthday) {
                        date_of_birth_ymd = new Date(parseInt(profile.birthday));
                    }
                    if(!profile.public_uid){
                        profile.public_uid = uuid.v4();
                    }
                    ...
                    if (!user) {
                        ...
                        userPromise = db.User.create({
                            ...
                            date_of_birth_ymd: date_of_birth_ymd,
...
function findOrCreateSocialLoginUser(socialNetworkName, email, socialNetworkUUID, displayName, firstname, lastname, gender, date_of_birth) {
    return new Promise(function (resolve, reject) {
        finder.findUserByLocalAccountEmail(email).then(function (localLogin) {
                if (localLogin) {
                    if (localLogin.verified) {
                        db.SocialLogin.findOne({
                            ...
                            return db.sequelize.transaction(function (transaction) {
                                ...
                                var date_of_birth_ymd;
                                if (date_of_birth) {
                                    date_of_birth_ymd = new Date(parseInt(date_of_birth));
                                }
                                if (!socialLogin) {
                        ...
                        // The user has an unverified local login
                        // if he already have a social login then let's return it.
                        ...
                        return db.SocialLogin.findOne({
                            ...
                            if (socialLogin){
                                return resolve (localLogin.User);
                            } else {
                    ...
                    db.SocialLogin.findOne({
                        ...
                        if (existingSocialLogin) {
                            ...
                            // Does the user have another social login?
                            return finder.findUserBySocialAccountEmail(email).then(function (socialLogin) {
                                if (socialLogin) {
                                    user = socialLogin.User;
                                }
                                return db.sequelize.transaction(function (transaction) {
...
function fillProfile(user, display_name, firstname, lastname, gender, date_of_birth, transaction) {
    ...
    if (!user.date_of_birth && date_of_birth) {
        user.date_of_birth = parseInt(date_of_birth);
        user.date_of_birth_ymd = dateFormat(new Date(parseInt(date_of_birth)), "yyyy-mm-dd");
    }
...
function afterSocialLoginSucceed(req, res) {
    var redirect;
    var withCode = req.session.withcode;
    // Mail (i.e. login) is set to null since login via social login
    afterLoginHelper.afterLogin(req.user, null, res);

    delete req.session.withcode;

    try {
        if (req.body.state){
            let payload = JSON.parse(decodeURIComponent(req.body.state));
            redirect = payload.afterLoginRedirect;
            withCode = withCode || payload.withCode;
        }
    } catch (e) {
        // nothing: it not a valid json => it doesn't contain any redirect
    }
    ...
    if (! redirect) {
        if (req.session.loginRedirect) {
            ...
            redirect = requestHelper.getPath('/user/profile');
        }
    }

    if (withCode) {
        res.redirect(requestHelper.getPath('/api/v2/session/cookie?redirect=' + redirect));
    } else {
