    ...
    db.SocialLogin.findOne({
        ...
        if (!socialLogin) {
            ...
            db.LocalLogin.findOne({
                where: db.sequelize.where(db.sequelize.fn('lower', db.sequelize.col('login')), {$like: profile.email}),
                include: [db.User]
...
function findOrCreateSocialLoginUser(socialNetworkName, email, socialNetworkUUID, displayName, firstname, lastname, gender, date_of_birth) {
    return new Promise(function (resolve, reject) {
        db.LocalLogin.findOne({
            where: db.sequelize.where(db.sequelize.fn('lower', db.sequelize.col('login')), {$like: email.toLowerCase()}),
            include: [db.User]
        }).then(function (localLogin) {
                if (localLogin) {
