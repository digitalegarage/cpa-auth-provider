    ...
    db.SocialLogin.findOne({
        ...
        if (!socialLogin) {
            ...
            db.LocalLogin.findOne({
                where: db.sequelize.where(db.sequelize.fn('lower', db.sequelize.col('login')), {[Op.like]: profile.email.toLowerCase()}),
                include: [db.User]
                ...
                if (localLogin) {
                    ...
                    if (profile.email) {
                        ...
                        var where = {[Op.and]: []};
                        where[Op.and].push(db.sequelize.where(db.sequelize.fn('lower', db.sequelize.col('email')), {[Op.like]: profile.email.toLowerCase()}));
