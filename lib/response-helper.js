...
module.exports = function (logger) {
    ...
    var redirectError = function (redirectUri, error, description, state) {
        logger.debug(description);
        var destinationUri = redirectUri + '?error=' + encodeURIComponent(error);
        if (description && description !== '') {
            destinationUri += '&error_description=' + encodeURIComponent(description);
        }
        if (state && state !== '') {
            destinationUri += '&state=' + encodeURIComponent(state);
        }
