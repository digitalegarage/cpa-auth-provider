    ...
    req.getValidationResult().then(function (result) {
        if (!result.isEmpty()) {
            ...
            for (let i = 0; i < errors.length; i++) {
                causes.push(apiErrorHelper.buildErrors(errors[i].msg));
            }
...
function password_recover(req, res, next) {


    if (req.recaptcha.error) {
        next(apiErrorHelper.buildError(400, "INVALID_RECAPTCHA", "invalid recaptcha", req.__('API_PASSWORD_RECOVER_SOMETHING_WRONG_RECAPTCHA')));
        return;
    }
    ...
    req.getValidationResult().then(function (result) {
        if (!result.isEmpty()) {
            next(apiErrorHelper.buildError(400, 'INVALID_EMAIL', '\'' + req.body.email + '\' is not a valid email', req.__('API_PASSWORD_RECOVER_SOMETHING_WRONG_RECAPTCHA')));
        } else {
            return db.LocalLogin.findOne({
                ...
                if (localLogin) {
                    return codeHelper.generatePasswordRecoveryCode(localLogin.user_id)
                    .then((code) => {
                        emailHelper.send(
                            config.mail.from,
                            localLogin.login,
                    ...
                    next(apiErrorHelper.buildError(400, "USER_NOT_FOUND", "Cannot find an account with email '"+req.body.email+"' as local login", req.__('API_PASSWORD_RECOVER_USER_NOT_FOUND')));
                }
            })
            .catch((err)=> {
                next(err);
...
function getProfileByReq(req,ruser) {
    return new Promise((resolve,reject) => {
        ...
        .catch(e => {
            logger.error(e);
            reject(apiErrorHelper.buildError(400, 'UNABLE_TO_FETCH_PROFILE','Unable to fetch profile.'));
        });
    });
}