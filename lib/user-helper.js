...
function updateProfileV2(user, data) {
    return new Promise(
        function (resolve, reject) {
            var fields = ['firstname', 'lastname', 'gender', 'date_of_birth_ymd', 'language'];
            var attributes = {};
            fields.forEach(function (key) {
                if (data.hasOwnProperty(key)) {
                    //use XSS filters to prevent users storing malicious data/code that could be interpreted then
            ...
            if (attributes.date_of_birth_ymd) {
            ...
            if (attributes.firstname || attributes.lastname) {
                ...
                attributes.displayName = user.email;
            }
...
function validateProfileUpdateDataV2(req) {
    if (requiredFields.firstname) {
        req.checkBody('firstname', req.__('BACK_PROFILE_UPDATE_FIRSTNAME_EMPTY_OR_INVALID')).notEmpty().matches(NAME_REGEX);
    } else if (req.body.firstname) {
        req.checkBody('firstname', req.__('BACK_PROFILE_UPDATE_FIRSTNAME_EMPTY_OR_INVALID')).matches(NAME_REGEX);
    ...
    if (requiredFields.date_of_birth_ymd) {
        req.checkBody('date_of_birth_ymd', req.__('BACK_PROFILE_UPDATE_DATE_OF_BIRTH_EMPTY_OR_INVALID')).notEmpty().isValidDate();
    } else if (req.body.date_of_birth_ymd) {
        req.checkBody('date_of_birth_ymd', req.__('BACK_PROFILE_UPDATE_DATE_OF_BIRTH_EMPTY_OR_INVALID')).isValidDate();
    }
    if (requiredFields.gender) {
        req.checkBody('gender', req.__('BACK_PROFILE_UPDATE_GENDER_EMPTY_OR_INVALID')).notEmpty().isIn(['male', 'female', 'other']);
    } else if (req.body.gender) {
