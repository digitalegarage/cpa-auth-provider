...
function password_update(req, res, next) {

    req.checkBody('password', "PWD_EMPTY").notEmpty();
    req.checkBody('code', "CODE_MISSING").notEmpty();
    ...
    req.getValidationResult().then(function (result) {
        if (!result.isEmpty()) {
            ...
            for (let i = 0; i < errors.length; i++) {
                causes.push(apiErrorHelper.buildFieldError(errors[i].param, apiErrorHelper.TYPE.BAD_FORMAT_OR_MISSING, null, "First validation pass error: "+ errors[i].msg));
            }

            next(apiErrorHelper.buildError(400, "DATA_VALIDATION_ERROR", "They might be several causes see errors array", null, causes));
        }
        ...
        if (!passwordHelper.isStrong(req.body.email, req.body.password)) {
            ...
            for (let i = 0; i < weaknesses.length; i++) {
                errors.push(apiErrorHelper.buildFieldError("password", apiErrorHelper.TYPE.CUSTOM, weaknesses[i].code));
            }
            next(apiErrorHelper.buildError(400, "PASSWORD_WEAK", "Password week, they might be several causes see errors array", req.__('PASSWORD_WEAK'), errors));
                ...
                if (localLogin && localLogin.User) {
                    return codeHelper.recoverPassword(localLogin.User, req.body.code, req.body.password).then(function (success) {
                        if (success) {
                            ...
                            next(apiErrorHelper.buildError(400, "WRONG_RECOVERY_CODE", "Recovery code for that user is wrong or doesn't exists", req.__('BACK_PWD_WRONG_RECOVERY_CODE')));
                    ...
                    next(apiErrorHelper.buildError(400, "NO_USER_FOR_THIS_MAIL", "No user found for the following email '" + req.body.email+"'", req.__('BACK_PWD_UPDATE_USER_NOT_FOUND')));
                }
            }, function (error) {
                next(error);
            });
...
function password_recover(req) {
    ...
    return new Promise(function(resolve,reject) {
        ...
        if (req.recaptcha.error) {
            errors.push(apiErrorHelper.buildFieldError("g-recaptcha-response", apiErrorHelper.TYPE.BAD_FORMAT_OR_MISSING, null, "Bad recaptcha", req.__('API_PASSWORD_RECOVER_SOMETHING_WRONG_RECAPTCHA')));
        }
        ...
        .then(function (result) {
            if (!result.isEmpty()) {
                result.array().map(r => {error.push(apiErrorHelper.buildFieldError(r.param, apiErrorHelper.TYPE.BAD_FORMAT_OR_MISSING, null, r.msg));});
            }

            if (errors.length) {
                reject(apiErrorHelper.buildError(400, 'BAD_DATA', 'they might be several causes see errors array', errors));
                return;
            }
