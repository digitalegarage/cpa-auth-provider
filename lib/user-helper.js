...
function createLocalLogin(email, password, requiredAttributes, optionnalAttributes) {
    return new Promise(
        function (resolve, reject) {
            ...
            db.LocalLogin.findOne({where: db.sequelize.where(db.sequelize.fn('lower', db.sequelize.col('login')), {[Op.like]: email.toLowerCase()})}).then(
                function (localLogin) {
                    if (localLogin) {
                        throw new Error(EXCEPTIONS.EMAIL_TAKEN);
                    }
                    return db.SocialLogin.findOne({where: db.sequelize.where(db.sequelize.fn('lower', db.sequelize.col('email')), {[Op.like]: email.toLowerCase()})});
                }).then(
