    ...
    req.getValidationResult().then(function (result) {
        if (!result.isEmpty()) {
            ...
            for (let i = 0; i < errors.length; i++) {
                causes.push(errorHelper.buildErrors(errors[i].msg));
            }

            return res.status(400).json(errorHelper.buildError(400, "DATA_VALIDATION_ERROR", "They might be several causes see errors array", null, causes)).send();
        }
                ...
                if (localLogin && localLogin.User) {
                    return codeHelper.recoverPassword(localLogin.User, req.body.code, req.body.password).then(function (success) {
                        if (success) {
                            ...
                            return res.status(400).json(errorHelper.buildError(400, "WRONG_RECOVERY_CODE", "Recovery code for that user is wrong or doesn't exists", req.__('BACK_PWD_WRONG_RECOVERY_CODE'))).send();
                    ...
                    return res.status(400).json(errorHelper.buildError(400, "NO_USER_FOR_THIS_MAIL", "No user found for the following email '" + req.body.email+"'", req.__('BACK_PWD_UPDATE_USER_NOT_FOUND'))).send();
                }
            }, function (error) {
                next(error);
            });
