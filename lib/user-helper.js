    ...
    req.getValidationResult().then(function (result) {
        if (!result.isEmpty()) {
            ...
            next(apiErrorHelper.buildError(400, 
                ...
                '',
                [ apiErrorHelper.buildErrors(
                    'API_PASSWORD_RECOVER_ERROR.VALIDATION', 
                    'Validation error.')
                ]));
        ...
        db.LocalLogin.findOne({
            ...
            if (localLogin) {
                ...
                next(apiErrorHelper.buildError(400, 
                    ...
                    [ apiErrorHelper.buildErrors(
                        'API_PASSWORD_RECOVER_ERROR.USER_NOT_FOUND',
                        'User not found.', 
                        req.__('API_PASSWORD_RECOVER_USER_NOT_FOUND'))
                    ]));
            }
            ...
            next(apiErrorHelper.buildError(
                ...
                req.__('API_ERROR'), 
                [apiErrorHelper.buildErrors('INTERNAL_SERVER_ERROR.API_PASSWORD_RECOVER_ERROR','Password recover error.', '', error)]
                )
            );
        });
    });
