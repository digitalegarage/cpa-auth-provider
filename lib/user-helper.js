...
function password_update(req, res, next) {
    ...
    req.getValidationResult().then(function (result) {
        if (!result.isEmpty()) {
            ...
            apiErrorHelper.throwError(400, "DATA_VALIDATION_ERROR", "They might be several causes see errors array", null, causes);
        }
                ...
                if (localLogin && localLogin.User) {
                    return codeHelper.recoverPassword(localLogin.User, req.body.code, req.body.password).then(function (success) {
                        if (success) {
                            ...
                            apiErrorHelper.throwError(400, "WRONG_RECOVERY_CODE", "Recovery code for that user is wrong or doesn't exists", req.__('BACK_PWD_WRONG_RECOVERY_CODE'));
                    ...
                    apiErrorHelper.throwError(400, "NO_USER_FOR_THIS_MAIL", "No user found for the following email '" + req.body.email+"'", req.__('BACK_PWD_UPDATE_USER_NOT_FOUND'));
                }
            }, function (error) {
                next(error);
            });
