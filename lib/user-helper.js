...
var socialLoginHelper = require ('./social-login-helper');
...
module.exports = {
    ...
    findByLocalAccountEmail:findByLocalAccountEmail,
...
function createLocalLogin(email, password, requiredAttributes, optionnalAttributes) {
    return new Promise(
        function (resolve, reject) {
            ...
            findByLocalAccountEmail(email).then(
                function (localLogin) {
                    if (localLogin) {
                        throw new Error(EXCEPTIONS.EMAIL_TAKEN);
                    }
                    return socialLoginHelper.findBySocialAccountEmail(email);
                }).then(
